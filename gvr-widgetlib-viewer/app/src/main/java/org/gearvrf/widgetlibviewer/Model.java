...
//        model = context.getAssetLoader().loadModel("models/" + location);
    ...
    public void applyCustomShader(int index) {
        switch (index) {
            case 0:
                for (int i = 0; i < getRenderDatas().size() && i < mOriginalMaterial.size(); i++) {
                    getRenderDatas().get(i).setMaterial(mOriginalMaterial.get(i));
                    getRenderDatas().get(i).setCullFace(GVRRenderPass.GVRCullFaceEnum.Back);
                    getRenderDatas().get(i).setDrawMode(4);
                }
