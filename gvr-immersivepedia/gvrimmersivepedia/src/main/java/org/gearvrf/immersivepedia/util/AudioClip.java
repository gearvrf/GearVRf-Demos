...
public class AudioClip {
	...
	private static int uiLoadSoundID;
	...
	public static synchronized AudioClip getInstance(Context androidContext) {
		if (instance == null) {
			instance = new AudioClip(10, AudioManager.STREAM_MUSIC, 0,
					androidContext);
		}
		return instance;
	}

	private AudioClip(int maxStreams, int streamType, int srcQuality,
			Context context) {
	...
	public int playSound(int soundID, float leftVolume, float rightVolume) {
		...
		if (loaded) {
			streamID = soundPool.play(soundID, leftVolume, rightVolume, 1, 0,
					1f);
			priority = priority++;
			plays = true;
		}
	...
	public int playLoop(int soundID, float leftVolume, float rightVolume) {
		int streamID = 0;
		if (loaded) {
			streamID = soundPool.play(soundID, leftVolume, rightVolume, 1, -1,
					1f);
			priority = priority++;
			plays = true;
		}
	...
	private void loadinSounds() {
		uiImageCloseSoundID = soundPool.load(context,
				R.raw.sfx_ui_image_close_4_1, 1);
		uiImageOpenSoundID = soundPool.load(context,
				R.raw.sfx_ui_image_open_4_1, 1);
		uiLoadSoundID = soundPool.load(context, R.raw.sfx_ui_loading_1, 1);
		uiMenuHoverSoundID = soundPool.load(context,
				R.raw.sfx_ui_menu_hover_2_2, 1);
		uiMenuSelectSoundID = soundPool.load(context,
				R.raw.sfx_ui_menu_select_1_1, 1);
		uiMenuSelectWrongSoundID = soundPool.load(context,
				R.raw.sfx_ui_menu_select_wrong_1_1, 1);
		uiRotateSoundID = soundPool.load(context, R.raw.sfx_ui_rotate_1_1, 1);
		uiTextAppearSoundID = soundPool.load(context,
				R.raw.sfx_text_appear_4_1, 1);
		uiTextDisappearSoundID = soundPool.load(context,
				R.raw.sfx_text_disappear_4_1, 1);
